{'Actors': ['Jena Sims', 'Jeremy Renner', 'Robert Patrick', 'Robert Pralgo'],
 'Added': '2015-01-12',
 'Countries': ['USA'],
 'Description': '\n'
                'Kill the Messenger: A reporter becomes the target of a '
                'vicious smear campaign that drives him to the point of '
                "suicide after he exposes the CIA's role in arming Contra "
                'rebels in Nicaragua and importing cocaine into California. '
                'Based on the true story of journalist Gary Webb. ',
 'Director': ['Michael Cuesta'],
 'EntryPath': 'W:\\Portfolio\\PersonalWebsite\\Favorites/Movies/snapshot//Kill-the-Messenger',
 'EntryURL': 'https://www.imdb.com/title/tt1216491',
 'Entry_json': 'W:\\Portfolio\\PersonalWebsite\\Favorites/Movies/snapshot//Kill-the-Messenger/entry.json',
 'Entry_php': 'W:\\Portfolio\\PersonalWebsite\\Favorites/Movies/snapshot//Kill-the-Messenger/info.php',
 'Entry_py': 'W:\\Portfolio\\PersonalWebsite\\Favorites/Movies/snapshot//Kill-the-Messenger/info.py',
 'Genres': ['Biography', 'Crime', 'Drama', 'Mystery', 'Thriller'],
 'IMDB': 'http://www.imdb.com/title/tt1216491',
 'PrimeWire': 'http://www.primewire.ag/watch-1906091-Kill-the-Messenger',
 'Ratings': {'IMDB': '7.0/10',
             'Metascore': '60%',
             'PrimeWire': '4.25/5',
             'RT': '77%',
             'Votes': '617'},
 'Released': '2014-10-09',
 'Runtime': '112 mins ',
 'ThumbURL': 'http://images.primewire.ag/thumbs/1906091_Kill_the_Messenger_2014.jpg',
 'Title': 'Kill-the-Messenger'}