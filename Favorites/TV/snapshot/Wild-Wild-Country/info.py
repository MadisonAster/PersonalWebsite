{'@context': 'http://schema.org',
 '@type': 'TVSeries',
 'EntryAdded': '06-03-2020',
 'EntryPath': 'Favorites/TV/snapshot/Wild-Wild-Country',
 'EntryURL': 'https://www.imdb.com/title/tt7768848',
 'Entry_json': 'Favorites/TV/snapshot/Wild-Wild-Country/entry.json',
 'Entry_py': 'Favorites/TV/snapshot/Wild-Wild-Country/info.py',
 'Entry_thumb': 'Favorites/TV/snapshot/Wild-Wild-Country/thumb.jpg',
 'actor': [{'@type': 'Person',
            'name': 'Ma Anand Sheela',
            'url': '/name/nm4222336/'},
           {'@type': 'Person', 'name': 'Osho', 'url': '/name/nm1099569/'},
           {'@type': 'Person',
            'name': 'Philip Toelkes',
            'url': '/name/nm6579788/'},
           {'@type': 'Person',
            'name': 'Jane Stork',
            'url': '/name/nm9716640/'}],
 'aggregateRating': [{'@type': 'AggregateRating',
                      'bestRating': '10.0',
                      'ratingCount': 22230,
                      'ratingValue': '8.2',
                      'worstRating': '1.0'}],
 'contentRating': 'TV-MA',
 'creator': [{'@type': 'Organization', 'url': '/company/co0152336/'}],
 'datePublished': '2018-03-16',
 'description': 'Wild Wild Country is a TV mini-series starring Ma Anand '
                "Sheela, Osho, and Philip Toelkes. When the world's most "
                'controversial guru builds an utopian city deep in the Oregon '
                'country, conflict with the locals escalates into a '
                'national...',
 'genre': ['Documentary', 'Crime'],
 'image': 'https://m.media-amazon.com/images/M/MV5BMzczOTU4NzkxMl5BMl5BanBnXkFtZTgwNTAxNTI5NDM@._V1_.jpg',
 'keywords': '1980s,true crime,guru,antelope oregon,reference to bhagwan shree '
             'rajneesh',
 'name': 'Wild Wild Country',
 'trailer': [{'@type': 'VideoObject',
              'description': "When the world's most controversial guru builds "
                             'a utopian city deep in the Oregon desert, '
                             'conflict with the locals escalates into a '
                             'national scandal. A true story.',
              'embedUrl': '/video/imdb/vi4281907225',
              'name': 'Official Trailer',
              'thumbnail': {'@type': 'ImageObject',
                            'contentUrl': 'https://m.media-amazon.com/images/M/MV5BYjQ4ZmJlZTUtNDI0ZS00OWU3LTk5NjAtZDE0OWRlOTAzOTE1XkEyXkFqcGdeQXVyMTkxNjUyNQ@@._V1_.jpg'},
              'thumbnailUrl': 'https://m.media-amazon.com/images/M/MV5BYjQ4ZmJlZTUtNDI0ZS00OWU3LTk5NjAtZDE0OWRlOTAzOTE1XkEyXkFqcGdeQXVyMTkxNjUyNQ@@._V1_.jpg',
              'uploadDate': '2018-02-28T21:20:11Z'}],
 'url': '/title/tt7768848/'}