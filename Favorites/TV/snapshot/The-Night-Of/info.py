{'@context': 'http://schema.org',
 '@type': 'TVSeries',
 'EntryPath': 'W:/Portfolio/PersonalWebsite/Favorites/TV/snapshot/The-Night-Of',
 'EntryURL': 'https://www.imdb.com/title/tt2401256',
 'Entry_json': 'W:/Portfolio/PersonalWebsite/Favorites/TV/snapshot/The-Night-Of/entry.json',
 'Entry_php': 'W:/Portfolio/PersonalWebsite/Favorites/TV/snapshot/The-Night-Of/info.php',
 'Entry_py': 'W:/Portfolio/PersonalWebsite/Favorites/TV/snapshot/The-Night-Of/info.py',
 'Entry_thumb': 'W:/Portfolio/PersonalWebsite/Favorites/TV/snapshot/The-Night-Of/thumb.jpg',
 'actor': [{'@type': 'Person', 'name': 'Riz Ahmed', 'url': '/name/nm1981893/'},
           {'@type': 'Person',
            'name': 'John Turturro',
            'url': '/name/nm0001806/'},
           {'@type': 'Person', 'name': 'Bill Camp', 'url': '/name/nm0131966/'},
           {'@type': 'Person',
            'name': 'Amara Karan',
            'url': '/name/nm2496312/'}],
 'aggregateRating': {'@type': 'AggregateRating',
                     'bestRating': '10.0',
                     'ratingCount': 110774,
                     'ratingValue': '8.5',
                     'worstRating': '1.0'},
 'contentRating': 'TV-MA',
 'creator': [{'@type': 'Person',
              'name': 'Richard Price',
              'url': '/name/nm0697115/'},
             {'@type': 'Person',
              'name': 'Steven Zaillian',
              'url': '/name/nm0001873/'},
             {'@type': 'Organization', 'url': '/company/co0231124/'},
             {'@type': 'Organization', 'url': '/company/co0304634/'},
             {'@type': 'Organization', 'url': '/company/co0618997/'},
             {'@type': 'Organization', 'url': '/company/co0208135/'},
             {'@type': 'Organization', 'url': '/company/co0008693/'}],
 'datePublished': '2016-06-24',
 'description': 'The Night Of is a TV mini-series starring Riz Ahmed, John '
                'Turturro, and Bill Camp. After a night of partying with a '
                'woman he picked up, a man wakes up to find her stabbed to '
                'death and is charged with her murder.',
 'genre': ['Crime', 'Drama', 'Mystery'],
 'image': 'https://m.media-amazon.com/images/M/MV5BMjQyOTgxMDI0Nl5BMl5BanBnXkFtZTgwOTE4MzczOTE@._V1_.jpg',
 'keywords': 'murder investigation,eczema,muslim,rikers island new york '
             'city,based on british tv series',
 'name': 'The Night Of',
 'review': {'@type': 'Review',
            'author': {'@type': 'Person', 'name': 'jql_longhurst'},
            'dateCreated': '2018-09-11',
            'inLanguage': 'English',
            'itemReviewed': {'@type': 'CreativeWork',
                             'url': '/title/tt2401256/'},
            'name': 'Sets a new bar',
            'reviewBody': 'Stunning, unpredictable, fresh, cynical. Perfect '
                          'casting and sub plots. Scarily close to the bone on '
                          'key social and religious tensions all around us. '
                          'Authentic, gripping, binge-worthy',
            'reviewRating': {'@type': 'Rating',
                             'bestRating': '10',
                             'ratingValue': '10',
                             'worstRating': '1'}},
 'trailer': {'@type': 'VideoObject',
             'description': 'After a night of partying with a female stranger, '
                            'a man wakes up to find her stabbed to death and '
                            'is charged with her murder.',
             'embedUrl': '/video/imdb/vi2516104985',
             'name': 'Tonight 9 p.m.',
             'thumbnail': {'@type': 'ImageObject',
                           'contentUrl': 'https://m.media-amazon.com/images/M/MV5BMTYxMjA5MzY1MF5BMl5BanBnXkFtZTgwNDI4MzczOTE@._V1_.jpg'},
             'thumbnailUrl': 'https://m.media-amazon.com/images/M/MV5BMTYxMjA5MzY1MF5BMl5BanBnXkFtZTgwNDI4MzczOTE@._V1_.jpg',
             'uploadDate': '2016-07-08T00:40:05Z'},
 'url': '/title/tt2401256/'}