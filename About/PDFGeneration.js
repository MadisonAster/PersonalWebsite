...
function AddProfileDetails(doc, PositionTitle, ycursor, headerspace) {
    ...
    doc.setFontStyle('normal');
    var xcursor = 250;
    doc.textWithLink(GetProfileURL(), xcursor, ycursor, {align: 'left', url:'https://'+GetProfileURL()});
    doc.setFillColor(51, 60, 67);
    doc.rect(xcursor, ycursor+2, doc.getTextWidth(GetProfileURL()), 0.25, 'F');
    xcursor += doc.getTextWidth(GetProfileURL());
    xcursor += 5;
    doc.setFontSize(10);
    doc.setFont('BabelStoneShapes');
    doc.textWithLink('\u21E6', xcursor, ycursor, {align: 'left'});
    xcursor += 15;
    doc.setFontSize(8);
    doc.setFont('mesmerize-el');
    doc.textWithLink('Really Cool Portfolio!', xcursor, ycursor, {align: 'left'});
    ycursor += 20;
    doc.setFontSize(12);
